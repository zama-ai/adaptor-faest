...
static void print_usage(const char *arg0) {
  ...
  printf("\n Valid instances are 1, 2, 3 -- see instances.cpp for details\n");
#endif
}

static void print_usage_free(const char *arg0) {
#if defined(_MSC_VER)
  printf("usage: %s iterations kappa lowmc_sboxes N tau\n", arg0);
#else
  printf("usage: %s [-i iterations] kappa aes_sboxes N tau\n", arg0);
  printf("\nIn the current implementation, you must set kappa=16 (for 128-bit security) aes_sboxes=200 (for AES-128)\n");
#endif
}
...
bool parse_args(bench_options_t *options, int argc, char **argv) {
  ...
  if (optind == argc - 1) {
    ...
    options->params = static_cast<params_t>(p);
  } else {
    print_usage(argv[0]);
    return false;
  }
