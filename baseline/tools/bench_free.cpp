...
struct timing_and_size_t {
  uint64_t keygen, sign, verify, compress, decrypt, size;
};

static void print_timings(const std::vector<timing_and_size_t> &timings) {
  timing_and_size_t average = {0, 0, 0, 0, 0, 0};
  printf("keygen,sign,verify,compress,decrypt,size\n");
  for (size_t i = 0; i < timings.size(); i++) {
    timing_and_size_t timing = timings[i];
    printf("%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 "\n",
           timing.keygen, timing.sign, timing.verify, timing.compress, timing.decrypt, timing.size);
    if (i > 0) {
      average.keygen   += timing.keygen;
      average.sign     += timing.sign;
      average.verify   += timing.verify;
      average.compress += timing.compress;
      average.decrypt  += timing.decrypt;
  ...
  printf("%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 "\n",
         ...
         average.compress / n, average.decrypt / n, average.size / n);
}
...
static int bench_sign_and_verify_free(const bench_options_free_t *options) {
  ...
  instance.commitment_size = COMMITMENT_SIZE;
  ...
  if (options->sboxes != 200) {
    printf("invalid sboxes, choose 200\n");
    return (-1);
  }
  instance.block_cipher_params = {16, 16, 200};
  instance.num_multiplications = 200;
  ...
  for (unsigned int i = 0; i != options->iter; ++i) {
    ...
    if(ret)
      std::cerr << "failed to generate VE keypair" << std::endl;

    std::vector<uint8_t> signature =
        helium_sign(instance, keypair, ve_pk, m, sizeof(m));
    ...
    start_time = timing_read(&ctx);
    bool ok = helium_verify(instance, keypair.second, signature, ve_pk, m, sizeof(m));
    tmp_time = timing_read(&ctx);
    ...
    start_time = timing_read(&ctx);
    std::vector<uint8_t> ctext = helium_compress(instance, signature);
    tmp_time = timing_read(&ctx);
    timing.compress = tmp_time - start_time;

    start_time = timing_read(&ctx);
    std::vector<uint8_t> decrypted = helium_decrypt(instance, ve_sk, ctext);
    tmp_time = timing_read(&ctx);
    timing.decrypt = tmp_time - start_time;
    if (decrypted != keypair.first)
