...
void commit_to_party_seed(const signature_instance_t &instance,
  ...
  assert(salt.data() >= PKE_RANDOMBYTES);
  assert(commitment.size() == instance.commitment_size);
  assert(commitment.size() == PKE_CIPHERTEXTOVERHEADBYTES + salt.size());
  (void) instance; // Unused in the VE implementation
  memcpy(rb, salt.data(), PKE_RANDOMBYTES);
  rb[0] = (uint8_t) rep_idx;
  rb[1] = (uint8_t) party_idx;  // We can only pass 32-bytes of randomness to PKE so we use 2 bytes for the counters and only 30 salt bytes
  assert(instance.num_MPC_parties <= 256);  // With more parties we need more than one byte for the party_idx
  assert(instance.repetitions <= 256);      // With more repetitions we need more than one byte for the rep_index
  int ret = pke_encrypt(ve_pk, commitment.data(), rb, seed.data(), seed.size());
  if(ret) {
    printf("pke_encrypt failed\n");
  }
...
void commit_to_4_party_seeds(
  ...
  // There is no 4-times encryption for our PKE, so we just commit individually.
  commit_to_party_seed(instance, ve_pk, seed0, salt, rep_idx, party_idx, com0);
  commit_to_party_seed(instance, ve_pk, seed1, salt, rep_idx, party_idx + 1, com1);
  commit_to_party_seed(instance, ve_pk, seed2, salt, rep_idx, party_idx + 2, com2);
  commit_to_party_seed(instance, ve_pk, seed3, salt, rep_idx, party_idx + 3, com3);
}
...
signature_t<GF_CHECK> helium_sign_template(const signature_instance_t &instance,
  ...
  // generate salt and root seeds for each repetition
  auto [salt, root_seeds] =
  ...
  RepByteContainer party_seed_commitments(
      instance.num_repetitions, instance.num_MPC_parties, instance.commitment_size);  

  for (size_t repetition = 0; repetition < instance.num_repetitions;
       repetition++) {
    // generate seed tree for the N parties
    seed_trees.emplace_back(root_seeds[repetition], instance.num_MPC_parties,
                            salt, repetition);
    ...
    // commit to each party's seed;
    {
      ...
      for (; party < instance.num_MPC_parties; party++) {
        commit_to_party_seed(
            instance, ve_pk, seed_trees[repetition].get_leaf(party).value(), salt,
            repetition, party, party_seed_commitments.get(repetition, party));
      }
    }
  ...
  for (size_t repetition = 0; repetition < instance.num_repetitions;
    ...
    std::vector<uint8_t> commitment(instance.commitment_size);
  ...
  for (size_t repetition = 0; repetition < instance.num_repetitions;
    ...
    // proof
    {
      ...
      for (; party < (instance.num_MPC_parties / 4) * 4; party += 4) {
        ...
        commit_to_4_party_seeds(
            instance, ve_pk, seed0, seed1, seed2, seed3, signature.salt, repetition,
      ...
      for (; party < instance.num_MPC_parties; party++) {
        if (party != missing_parties[repetition]) {
          commit_to_party_seed(instance, ve_pk,
                               seed_trees[repetition].get_leaf(party).value(),
...
template <typename GF_CHECK>
std::vector<uint8_t> helium_compress_template(const signature_instance_t &instance,
                           const signature_t<GF_CHECK> &signature){

  // Recreate seed trees and random tapes
  std::vector<SeedTree> seed_trees;
  const size_t random_tape_size =
      instance.block_cipher_params.key_size + instance.num_multiplications +
      (instance.num_multiplications - 2) + 3 * GF_CHECK::BYTE_SIZE;
  ...
  for (size_t repetition = 0; repetition < instance.num_repetitions;
    ...
    if (missing_parties[repetition] != proof.reveallist.second)
      throw std::runtime_error(
          "modified signature between deserialization and compress");
    seed_trees.push_back(SeedTree(proof.reveallist, instance.num_MPC_parties,
                                  signature.salt, repetition));
  ...
  // recompute shares of the block cipher key
  RepByteContainer rep_shared_keys(instance.num_repetitions,
                                   instance.num_MPC_parties,
                                   instance.block_cipher_params.key_size);
  RepByteContainer rep_w_tilde_e(instance.num_repetitions, 1, instance.block_cipher_params.key_size);
  RepByteContainer rep_com_e(instance.num_repetitions, 1, instance.commitment_size);


  for (size_t repetition = 0; repetition < instance.num_repetitions; repetition++) {
    const repetition_proof_t<GF_CHECK> &proof = signature.proofs[repetition];
    ...
    // Compute w_tilde_e
    auto w_tilde_e = rep_w_tilde_e.get(repetition, 0);
    for (size_t party = 0; party < instance.num_MPC_parties; party++) {
      if(party == missing_parties[repetition])
        continue; // skip missing party's share
      auto share = rep_shared_keys.get(repetition, party);
      // w_tilde_e = w_tilde_e XOR share
      std::transform(std::begin(w_tilde_e), std::end(w_tilde_e), 
                     std::begin(share), std::begin(w_tilde_e), 
                     std::bit_xor<uint8_t>());  
    }

    if(missing_parties[repetition] == 0) {
      // XOR in the fix for the first share, since it'll be missing during decryption 
      // as only the first party's seed (not share) is encrypted
      std::transform(std::begin(w_tilde_e), std::end(w_tilde_e), 
                     std::begin(proof.sk_delta), std::begin(w_tilde_e), 
                     std::bit_xor<uint8_t>());        
    }

    // Copy the missing commitment
    auto com_e = rep_com_e.get(repetition, 0);
    std::copy(std::begin(proof.Com_e), std::end(proof.Com_e), std::begin(com_e));
  }

  // Serialize salt, (missing_party_e, rep_w_tilde_e, rep_com_e)_{e \in tau}
  std::vector<uint8_t> serialized;
  const size_t ve_ctext_size = signature.salt.size() + instance.num_repetitions * (2 + instance.block_cipher_params.key_size + instance.commitment_size);
  serialized.reserve(ve_ctext_size);
  serialized.insert(serialized.end(), signature.salt.begin(),
                    signature.salt.end());  
  for (size_t repetition = 0; repetition < instance.num_repetitions;
       repetition++) {
    uint16_t party = (uint16_t) missing_parties[repetition];
    serialized.push_back(((uint8_t*)&party)[0]);
    serialized.push_back(((uint8_t*)&party)[1]);
    auto com_e = rep_com_e.get(repetition, 0);
    serialized.insert(serialized.end(), com_e.begin(), com_e.end());

    auto w_tilde_e = rep_w_tilde_e.get(repetition, 0);
    serialized.insert(serialized.end(), w_tilde_e.begin(), w_tilde_e.end());
  }  
  assert(ve_ctext_size == serialized.size());
  return serialized;
}
  ...
  const size_t signature_size =
      ...
      instance.num_repetitions * // tau x
          (ceil_log2(instance.num_MPC_parties) *
           ...
           instance.commitment_size +              // Com_e
...
helium_deserialize_signature(const signature_instance_t &instance,
  ...
  for (size_t repetition = 0; repetition < instance.num_repetitions;
    ...
    std::vector<uint8_t> Com_e(instance.commitment_size);
...
std::vector<uint8_t> helium_sign(const signature_instance_t &instance,
  ...
  if (instance.block_cipher_params.block_size == 16) {
    auto sig = helium_sign_template<USED_CHECK_FIELD>(instance, keypair, ve_pk, message,
                                                     message_len);
...
bool helium_verify(const signature_instance_t &instance,
  ...
  if (instance.block_cipher_params.block_size == 16) {
    ...
    return helium_verify_template<USED_CHECK_FIELD>(instance, pk, sig, ve_pk, message,
                                                   message_len);
  }
  throw std::runtime_error("parameter set not implemented");
}
...
std::vector<uint8_t> helium_compress(const signature_instance_t &instance, const std::vector<uint8_t> &signature_bytes) {
  if (instance.block_cipher_params.block_size == 16) {
    auto sig =
        helium_deserialize_signature<USED_CHECK_FIELD>(instance, signature_bytes);
    return helium_compress_template<USED_CHECK_FIELD>(instance, sig);
  }
  throw std::runtime_error("parameter set not implemented");
...
std::vector<uint8_t> helium_decrypt(const signature_instance_t &instance, const uint8_t* ve_sk, std::vector<uint8_t> &ve_ciphertext) {
  ...
  salt_t salt{};
  const size_t key_size = instance.block_cipher_params.key_size;
  const size_t ve_ctext_size = salt.size() + instance.num_repetitions * (2 + instance.block_cipher_params.key_size + instance.commitment_size);
  if(ve_ciphertext.size() < ve_ctext_size) {
    throw std::runtime_error("input ciphertext too short");
  }

  std::vector<uint8_t> witness(key_size);    

  size_t current_offset = 0; 
  memcpy(salt.data(), ve_ciphertext.data() + current_offset, salt.size());
  current_offset += salt.size();
  ...
  for (repetition = 0; repetition < instance.num_repetitions; repetition++) {
    ...
    uint16_t missing_party;
    memcpy(&missing_party, ve_ciphertext.data() + current_offset, 2);
    current_offset += 2;
    //std::cout << "Read missing party = " << missing_party << "\n";

    std::vector<uint8_t> com_e(instance.commitment_size);
    memcpy(com_e.data(), ve_ciphertext.data() + current_offset, instance.commitment_size);
    current_offset += instance.commitment_size;    
    //std::cout << "Read com_e\n";

    std::vector<uint8_t> w_tilde_e(key_size);
    memcpy(w_tilde_e.data(), ve_ciphertext.data() + current_offset, key_size);    
    current_offset += key_size;
    //std::cout << "Read w_tilde_e\n";

    std::vector<uint8_t> seed(key_size);
    int ret = pke_decrypt(ve_sk, com_e.data(), seed.data(), key_size);
    if(ret) {
      std::cout << "Failed to decrypt ciphertext at repetition " << repetition << "\n";
      continue;
    }

    // Now re-derive missing share from seed
    RandomTape tape(seed, salt, repetition, missing_party);
    std::vector<uint8_t> share(key_size);
    tape.squeeze_bytes(share.data(), key_size);

    // witness  = w_tilde_e XOR share
    std::transform(std::begin(w_tilde_e), std::end(w_tilde_e), 
                     std::begin(share), std::begin(witness), 
                     std::bit_xor<uint8_t>());  
    break;
  }

  if(repetition == instance.num_repetitions) {
    std::cout << "Decryption failed in all repetitions\n";    
    witness.clear();
  }
